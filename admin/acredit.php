	...
	<div class="col-md-12 grid-margin stretch-card">
                ...
                <div class="card-body">
                  <form class="forms-sample" method="POST" action="#">
					...
					<h3>Balance Amount</h3>
					  ...
					  <label for="exampleInputUsername1"><h4>Shop Name</h4></label>
					<select class="custom-select" name="sname">
                       ...
                       <label for="exampleInputUsername1"><h4>Amount</h4></label>
                      <input type="text" name="amount" class="form-control" id="exampleInputUsername1" placeholder="Amount">
					  </br>
                    </div>
			...
			<?php
                  if (isset($_POST['ADD'])) {
                    ...
                    include("config.php");
                    $sname = $_POST['sname'];
                    $query = "select * from credit where sname='".$sname."'";
                    ...
                    if ($count > 0) {
                      $query = "update credit set sname='" . $sname . "',amount=amount+'" . $amount . "' where sname='" . $sname . "'";
                      mysqli_query($con, $query) or die(mysqli_error($con));

                      $query = "update register set totalamt=totalamt-'" . $amount . "'";
                      mysqli_query($con, $query) or die(mysqli_error($con));

                      echo "<script>
				alert('Updated...');
			</script>";
                      echo "<script> location.href='acredit.php'; </script>";
                    } else {
                      $query1 = "insert into credit(sname,amount) values('" . $sname . "','" . $amount . "')";
                      mysqli_query($con, $query1) or die(mysqli_error($con));
                      $query = "update register set totalamt=totalamt-'" . $amount . "'";
                      mysqli_query($con, $query) or die(mysqli_error($con));
                      echo "<script>
				alert('Added Successfully Completed, .');
			</script>";
                      echo "<script> location.href='acredit.php'; </script>";
                    }
			...
			<div class="col-lg-12 grid-margin stretch-card">
              <div class="card">
                <div class="card-body">
                  <h4 class="card-title">Balance Amount Details</h4>
...
if(isset($_POST['delete']))
{
	...
	$query1 = "delete from credit where id='".$selectano."'";
	mysqli_query($con,$query1) or die(mysqli_error($con));
	echo "<script> location.href='acredit.php'; </script>";
}
?>
